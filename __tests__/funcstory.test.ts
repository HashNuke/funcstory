...
describe('FuncStory CLI', () => {
  ...
  describe('Error handling', () => {
    ...
    it('should error when file does not exist', async () => {
      process.argv = ['node', 'funcstory', 'nonexistent.ts', '--entry', 'test'];
    ...
    it('should error when entry point is not found', async () => {
      const testFile = path.join(fixturePath, 'simple-functions.ts');
      process.argv = ['node', 'funcstory', testFile, '--entry', 'nonexistentFunction'];
  ...
  describe('Function tracing', () => {
    ...
    it('should trace class methods', async () => {
      ...
      process.argv = ['node', 'funcstory', testFile, '--entry', 'Calculator.complexCalculation'];
    ...
    it('should skip functions marked with @funcstory-skip', async () => {
      const testFile = path.join(fixturePath, 'simple-functions.ts');
      process.argv = ['node', 'funcstory', testFile, '--entry', 'functionThatCallsSkipped'];
    ...
    it('should detect recursive functions', async () => {
      const testFile = path.join(fixturePath, 'simple-functions.ts');
      process.argv = ['node', 'funcstory', testFile, '--entry', 'factorial'];
    ...
    it('should not include JSDoc information in oneline mode', async () => {
      const testFile = path.join(fixturePath, 'class-methods.ts');
      process.argv = ['node', 'funcstory', testFile, '--entry', 'Calculator.complexCalculation', '--oneline'];
    ...
    it('should respect max depth option', async () => {
      const testFile = path.join(fixturePath, 'simple-functions.ts');
      process.argv = ['node', 'funcstory', testFile, '--entry', 'factorial', '--max-depth', '2'];
  ...
  describe('Argument parsing', () => {
    it('should handle unknown arguments', async () => {
      const testFile = path.join(fixturePath, 'simple-functions.ts');
      process.argv = ['node', 'funcstory', testFile, '--entry', 'test', '--unknown'];
